...
## v22 — 인기 종목 리스트 고도화
- **출처 표기(신뢰도)**: 인기 종목 상단에 `Data Source: Yahoo Finance · 최대 15분 지연` 9px 회색으로 명시(실제 소스에 맞춰 정직하게).
- **Progressive 더보기**: 63개 유니버스에서 **10개씩** 표시, "더보기(+10)"로 확장 → 초기 로딩 부담↓.
- **LIVE 뱃지 + 30초 갱신**: 초록 점 깜빡이는 LIVE 배지, 30초마다 랭킹 재정렬/갱신.
- **네이티브 광고**: 리스트 **5위와 6위 사이**에 [Sponsored] 카드 삽입(팝업 아님, 콘텐츠형).
- (기술) Yahoo spark 10개 제한을 서버에서 10개씩 자동 분할 호출로 해결 → movers 히트맵 등 다른 배치 시세도 안정화.
- 제목: "글로벌 시장, 한눈에" → **"Market Pulse · 시장의 흐름"**.

## v23 — 기술적 지표 + 멀티 타임프레임 차트
- **타임프레임**: 1분 · 5분 · 15분 · 1시간 · 일 · 주 · 월 (Yahoo 무료, OHLC+거래량).
- **지표 토글**: 거래량 · MA(20/60) · EMA(12/26) · 볼린저밴드(20,2) · VWAP · RSI(14) · MACD(12/26/9). 종가 기준 클라이언트 계산.
- 가격 패널에 오버레이(MA/EMA/볼린저/VWAP), 하단에 거래량·RSI·MACD 서브패널이 켠 만큼 자동으로 쌓임. 점 툴팁에 값 표시. 지표 참고용 면책 표기.

## v24 — AI 뉴스 센터 (상단 메뉴 "뉴스")
- **컴플라이언스**: 라이선스 API(Finnhub 우선 → Yahoo 폴백)만 사용. 제목·출처·시간·관련종목·원문링크만 표시하고 기사 전문은 저장/재배포하지 않음. 요약·분석은 AI 생성.
- **Top News + 카드 그리드**: 첫 항목을 크게, 나머지는 카드로. 토스/블룸버그 느낌.
- **자동 처리**: 중복 뉴스 제거, 클릭베이트/스팸 필터, 카테고리 자동 분류(Market·Earnings·Economy·Technology·AI·Crypto), 제목 기반 감성(긍정/부정/중립) 뱃지.
- **카테고리 필터 + 무한 더보기(12개씩)** + 스켈레톤 로딩 + LIVE 뱃지.
- **AI 요약·분석**: 각 카드의 "✨ AI 요약·분석" 버튼으로 해당 헤드라인을 AI가 요약(ai.js news_summary). 관련 종목 칩 클릭 시 종목 분석으로 이동.
- **근실시간**: WebSocket은 백엔드가 필요해 무료 범위에선 90초 폴링으로 자동 갱신(탭 숨기면 정지). 종목 상세의 [뉴스] 탭은 기존처럼 해당 기업 뉴스만 표시.

## v26 — 실시간 시장 대시보드 강화
- **실시간 상태줄**: 홈 상단에 초록/노랑/빨강 점 + "실시간 · HH:MM:SS 업데이트 · 15초마다 자동". 클릭 시 즉시 새로고침.
- **자동 갱신 15초**(기존 30초): 지수·크립토·인기 종목 랭킹 + AI 시장 점수까지 실시간 반영, 값 변동 시 초록/빨강 글로우.
- **자동 재연결**: 갱신 실패 시 "재연결 중…" 표시 후 8초 뒤 재시도(최대 3회 백오프), 성공하면 자동 복구.
- **탭 절전**: 다른 탭으로 가면 폴링 정지, 돌아오면 즉시 1회 갱신(불필요한 호출·배터리 절약).
- (무료 범위라 WebSocket 대신 폴링 기반의 근실시간입니다.)

## v29 — 인기 종목: 등락률 → 거래량(관심도) 중심 전환
- **정렬**: 상승/하락률 순 → **당일 거래량 순**(투자자 시선이 쏠린 종목).
- **전면 지표**: 카드 우측에 **거래량**을 크게(예: "1.23억 주"), 등락률은 작은 보조로.
- **문구**: "오늘 투자자 시선이 가장 많이 쏠린 종목 · 당일 거래량 순", 라벨 "등락 상위 → 거래량 순".
- 데이터: Yahoo 차트 메타의 당일 거래량(무키). 서버 30초 캐시, 폴링은 60초마다만 랭킹 갱신(부하 방지).
- 광고 카드 문구도 "상승 유망주 → 지금 가장 활발한 종목"으로 정정(거래량 1위가 하락 종목일 수도 있으므로).

## v31 — 페이월(Paywall) UI + 수익화 메모
- **블러 페이월**: 비회원에게 PRO 콘텐츠(투자 시나리오, AI 포트폴리오 진단)를 블러 티저 + 🔒 오버레이 + "잠금 해제 →"(모달)로 노출. 티저는 실제 분석이 아닌 placeholder(●)로, 오해 방지.
- **무료/PRO 프롬프트 분기**: 이미 `api/ai.js`의 `premium` 파라미터로 무료=간결/PRO=심층 분기 처리됨(기존).
- 결제: 현재는 코드-언락(수동)입니다. Stripe/토스페이먼츠 정기결제는 백엔드+결제사 연동이 필요(정적 앱만으로는 불가). 카드 정보는 앱에서 직접 다루지 않고 결제사 페이지로 위임하세요.
- ⚠️ 컴플라이언스: "강력 매수·적정 진입가·손절가·매물대 돌파 시그널" 같은 개인 맞춤 매매 지시는 유사투자자문 소지가 있어 앱 원칙상 자동 생성하지 않습니다. AI는 가이던스·리스크 요약 등 '참고 정보'로 유지하고 면책 문구를 표시합니다.

## v32 — 광고/제휴(1단계) + GA4(2단계)
설정은 `index.html` 상단 스크립트의 상수에 입력(비워두면 아무것도 로드되지 않고 앱도 정상):
- `ADSENSE_CLIENT`, `ADSENSE_SLOT` → 채우면 개요 탭 광고 자리에 실제 애드센스 인피드 광고 노출("SPONSORED" 라벨).
- `GA4_ID` → 채우면 GA4 자동 로드 + 커스텀 이벤트 전송: `search_ticker`(검색), `click_premium_preview`(페이월 클릭=결제 의향), `read_ai_summary_fully`(AI 분석 10초 체류).
- `AFFILIATE.broker/book.link` → 채우면 AI 분석 아래 **문맥 기반 제휴 배너**(AI 텍스트에 재무·리스크 키워드면 도서, 아니면 증권사) 노출. 링크 비면 자동 숨김. `rel="sponsored"` + "· 광고" 라벨로 정직 표기.

⚠️ 개인정보/규정: GA4·애드센스는 쿠키를 씁니다. EU/한국 대상이면 **쿠키 동의 배너·개인정보처리방침**이 필요합니다(애드센스 정책 필수). 제휴 링크는 "광고/제휴"임을 표기하세요(이미 반영). 투자 매매 시그널(진입가·손절)은 앱 원칙상 미제공 유지.

## v33 — 실제 값 입력 + ads.txt + 쿠키 동의 + 개인정보처리방침
- **입력 완료**: `GA4_ID="G-9GQYWQ2GEQ"`, `ADSENSE_CLIENT="ca-pub-9617531470345660"`. AdSense 인증 스크립트를 `<head>`에 추가(사이트 "검토 필요" 확인용). 광고 자리 실제 노출은 승인 + 광고단위 `ADSENSE_SLOT` 입력 후.
- **ads.txt 생성**: `google.com, pub-9617531470345660, DIRECT, f08c47fec0942fa0` → 저장소 루트에 두면 `astroiron.com/ads.txt`로 서빙되어 "Ads.txt 찾을 수 없음"이 해결됩니다.
