========================================
 AstroIron v1.9 — 배포용 통합 패키지 (애드센스 준비 완성본)
 생성일: 2026-07-13 · 표준 ZIP (반디집으로 열림)
========================================
 [v1.9] 뉴스 제목 한국어 번역 — LLM 미사용 무료 번역 서버 신규.
        api/translate.js: Google 무키 엔드포인트 → MyMemory(무료) 폴백.
        키 불필요. 뉴스 화면 "🇰🇷 한국어" 버튼이 이제 무료로 동작.
        ※ 배포 후 점검: https://astroiron.com/api/translate?health=1
 [v1.8] 한국어 번역 보완(AI 미사용) — 뉴스 카테고리 정적 한글화
        (Market→시장, Earnings→실적, Economy→경제, Technology→기술, Crypto→암호화폐),
        "TOP NEWS"→"주요 뉴스", 광고 라벨 "SPONSORED/Sponsored"→"광고".
        ※ 뉴스 '제목' 번역은 자유형 텍스트라 AI 없이는 불가 → UI 라벨만 정적 번역.
 [v1.6] "토론방" → "커뮤니티"로 명칭 통일 (앱 내비/헤딩 + 개인정보처리방침·이용약관).
 [v1.7] 접근성·완성도 다듬기 — 본문 바로가기(skip link), 스크린리더
        aria 라벨(로고·FAB·닫기·음소거), 탭 aria-selected, 토스트 aria-live,
        prefers-reduced-motion(모션 최소화), 링크/버튼 포커스 표시.
